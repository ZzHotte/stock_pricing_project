...
def read_prc_csv(tic):
    ...
    # set the index into datetime type
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    """
    # <COMPLETE THIS PART>
    # set up a list to store dataframes
    df_lst = []

    for tic in tickers:
        tic = tic.lower()
        df = read_prc_csv(tic)
        # extract price column as single df with ticker as col_index
        df[tic] = df[prc_col]
        prc_col_df = df[[tic]]
    ...
    # join the list of dataframes with the empty one
...
def mk_ret_df(prc_df):
    ...
    """
    # <COMPLETE THIS PART>
    # calculate the percentage price change of the input df
    prc_df.sort_index(inplace=True)
    prc_df = prc_df.pct_change()

    # obtain the market return
    pth = cfg.FF_CSV
    ff_df = pd.read_csv(pth, index_col='Date')
    ff_df.index = pd.to_datetime(ff_df.index, format='%Y-%m-%d')

    # extract the mkt return into df
    mkt_prc_change = ff_df[['mkt']]

    # join mkt return into stock return df
...
def mk_aret_df(ret_df):
    ...
    """
    # <COMPLETE THIS PART>
    # get all stock tickers in a list except 'mkt'
    col_lst = list(ret_df.columns)
    firm_lst = col_lst[:-1]

    empty = ret_df.copy()

    # calculate the abnormal return for each stock
    ...
    # don't need to drop the rows with all cells of NaN
...
def get_avg(df, col, year):
    ...
    """
    #<COMPLETE THIS PART>
    # clean the df in case it's not in standard format
    df.index = pd.to_datetime(df.index, format='%Y-%m-%d')
...
def get_ann_ret(ser, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    # clean the data in case it's not in standard format
    ser.index = pd.to_datetime(ser.index, format='%Y-%m-%d')
    ser.sort_index(inplace=True)
    ser = ser.dropna()

    # extract data within holding period
    daily_ret = ser.loc[start:end]
    # print(daily_ret)
    # count the number of holding days N
